...
def _get_recommendations(opening_groups, phase_counts, difficulty="normal"):
    ...
    # Opening-based: top 3 blunder-prone openings
    for opening_name, opening_blunders in opening_groups[:3]:
        recs.append({
            ...
            "links": [
                ("Study on Lichess", f"https://lichess.org/opening/{opening_name.replace(' ', '_')}"),
            ],
        })
...
def _build_html(*, username, current_rating, total_games, wins, losses, draws,
    ...
    for b in all_blunders:
        if b.get("cp_loss") is not None:
            mn = b["move_num"]
            if mn not in mh: mh[mn] = {"cnt": 0, "total": 0}
            mh[mn]["cnt"] += 1
            mh[mn]["total"] += b["cp_loss"]
    move_chart = [{"m": m, "cnt": d["cnt"], "avg": round(d["total"] / d["cnt"])} for m, d in sorted(mh.items())]
    
    tl_data = [{"id": g["game_id"], "b": g["blunder_count"], "r": g["result"]} for g in sorted(recent_games, key=lambda x: x.get("date", 0))]
    ...
    for g in recent_games:
        ...
        phase_themes = {
            "opening": [("Opening puzzles", THEME_MAP["opening"])],
...
<div class="container">
    ...
    <!-- Lichess study export -->
    <div class="section">
        <div class="section-label">Export to Lichess study</div>
        <div style="background:var(--surface);border:1px solid var(--border);border-radius:8px;padding:16px 20px">
            <div style="font-size:13px;color:#fff;margin-bottom:8px;font-weight:500">Review your worst blunders on a board</div>
            <div style="font-size:12px;color:var(--dim);margin-bottom:14px;line-height:1.5">
                Create a Lichess study from your blunder games. Open the link below, then import these game IDs to analyze them with annotations.
            </div>
            <div style="display:flex;gap:8px;flex-wrap:wrap;margin-bottom:14px">
                <a href="https://lichess.org/study" target="_blank" style="font-family:var(--mono);font-size:11px;color:var(--teal);background:rgba(62,184,160,.08);border:1px solid rgba(62,184,160,.2);padding:6px 14px;border-radius:4px;text-decoration:none">Create new study</a>
            </div>
            <div style="font-family:var(--mono);font-size:11px;color:var(--faint);margin-bottom:4px">GAME IDS (paste into study import):</div>
            <div id="study-ids" style="font-family:var(--mono);font-size:11px;color:var(--dim);background:var(--bg);border:1px solid var(--border);border-radius:4px;padding:10px 14px;word-break:break-all;cursor:pointer;user-select:all">{' '.join(set(b['game_id'] for b in worst))}</div>
            <div style="font-size:11px;color:var(--faint);margin-top:6px;font-family:var(--mono)">Click to select, then paste into Lichess study import</div>
        </div>
    </div>
    ...
    <div class="section">
        ...
        <div style="margin-bottom:24px">
            ...
            <div style="font-size:12px;color:var(--dim);margin-bottom:12px">Your danger zone is the opening-to-middlegame transition</div>
            <div style="position:relative;width:100%;height:180px"><canvas id="ch_heat"></canvas></div>
        </div>
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:24px;margin-bottom:24px">
            <div>
                ...
                <div style="position:relative;width:100%;height:220px"><canvas id="ch_tl"></canvas></div>
            </div>
            <div>
    ...
    <div class="footer">
        ...
        <span style="margin-left:auto;display:flex;gap:16px">
            <a href="https://buymeacoffee.com/sajal" target="_blank" style="color:var(--accent2);text-decoration:none">☕ Buy me a coffee</a>
...
new Chart(document.getElementById('ch_heat'),{{type:'bar',data:{{labels:D.move.map(d=>'m'+d.m),datasets:[{{data:D.move.map(d=>d.cnt),backgroundColor:D.move.map(d=>{{const i=Math.min(d.avg/800,1);return'rgb('+Math.round(212*i+50*(1-i))+','+Math.round(68*i+50*(1-i))+','+Math.round(62*i+80*(1-i))+')';}}),borderRadius:2}}]}},options:{{responsive:true,maintainAspectRatio:false,plugins:{{legend:{{display:false}},tooltip:{{callbacks:{{label:c=>D.move[c.dataIndex].cnt+' blunders, avg '+D.move[c.dataIndex].avg+'cp'}}}}}},scales:{{y:{{grid:{{color:g}},ticks:{{stepSize:1}}}},x:{{grid:{{display:false}},ticks:{{maxRotation:90,font:{{size:9}}}}}}}}}}}});

new Chart(document.getElementById('ch_tl'),{{type:'bar',data:{{labels:D.tl.map((_,i)=>i+1),datasets:[{{data:D.tl.map(d=>d.b),backgroundColor:D.tl.map(d=>d.r==='win'?T:d.r==='loss'?R:A),borderRadius:3}}]}},options:{{responsive:true,maintainAspectRatio:false,plugins:{{legend:{{display:false}},tooltip:{{callbacks:{{label:c=>{{const t=D.tl[c.dataIndex];return t.id+': '+t.b+' blunders ('+t.r+')'}}}}}}}},scales:{{y:{{grid:{{color:g}},ticks:{{stepSize:2}}}},x:{{grid:{{display:false}},title:{{display:true,text:'oldest → newest',font:{{size:10}}}}}}}}}}}});
